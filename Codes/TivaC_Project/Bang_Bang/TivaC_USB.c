...
extern void take_action(uint8_t *ui8insrt, uint32_t ui32MsgLen);
...
uint32_t RxHandler(void *pvCBData, uint32_t ui32Event, uint32_t ui32MsgValue, void *pvMsgData)
{
    ...
    switch(ui32Event)
    {
        ...
        case USB_EVENT_RX_AVAILABLE:
        {
            ...
            g_ui32tmp0 = (get_data_from_host(psDevice, pvMsgData, ui32MsgValue));
            IntMasterEnable();
            return(g_ui32tmp0);
        }
                            ...
                            uint32_t ui32NumBytes)
{
  //SysCtlDelay(500);
    ...
    //SetLedColor(WHITE);
    // Enable Echo and return
    if(newData[0] <= 110)
    {
        g_i32SetTemp = (newData[0] << 12) / 500;
        g_i32SetTemp -= (10 << 12)/500 ;
        g_i32integral = 0;
        g_bTempStable = false;
        start_IV();
    }
    ...
    //decode_data(newData,ui32NumBytes);
    //return ui32NumBytes;
}
/*************************************************************************************************/
...
void initUSB()
{
    ...
    MAP_SysTickPeriodSet(ROM_SysCtlClockGet() / SYSTICKS_PER_SECOND);
